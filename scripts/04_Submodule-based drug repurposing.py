...
def GetCenterNodes(SD, mod):
    ...
    return [mod[idx] for idx, isCenter in enumerate(sum_ == sum_.min()) if isCenter]
...
class Network(object):
    ...
    def PrepareBins(self, binSize=150):
        ...
        for curr, next_ in zip(degrees, degrees[1:] + [degrees[-1] + 1]):
            for d in range(curr, next_):
                self.d2b[d] = index
        ...
        if len(self.b2i[index]) < binSize and index > 0:
    ...
    def DegreeMimicSampling(self, indexes):
        ...
        while True:
            ...
            for b in sorted(binCount.keys()):
                sampled_indexes.extend(random.sample(self.b2i[b], binCount[b]))
...
def ToDict(fp, Net):
    ...
    with open(fp, encoding="utf-8") as fi:
        for line in fi:
            ...
            if k.lower() in ["name", "drug", "", "disease", "module"]:
                continue

            if v.lower() in ["gene", "symbol", "target", "targets"]:
                continue
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(MAPPING1)} records from {FILE1}")
    ...
    for i, FILE2 in enumerate(input_files):
        ...
        print(f"Loaded {len(MAPPING2)} records from {FILE2}")

        with open(OUTPUT, "w", encoding="utf-8") as fo:


            fo.write("Source\tTarget\tDistance\tZ_score\tP_value\n")
